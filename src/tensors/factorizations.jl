...
only implemented for `spacetype(t)<:ElementarySpace`.
"""
...
"""
    eigh(t::AbstractTensorMap{<:ElementarySpace}, leftind::Tuple, rightind::Tuple) -> D, V
...
requires `spacetyp(t) <: ElementarySpace`.
...
"""
    isposdef(t::AbstractTensor{<:ElementarySpace}, leftind::Tuple, rightind::Tuple) -> ::Bool
...
#------------------------------------------------------------------------------------------
leftorth!(t::AdjointTensorMap{S}; alg::OFA = QRpos()) where {S<:ElementarySpace} =
    map(adjoint, reverse(rightorth!(adjoint(t); alg = alg')))

rightorth!(t::AdjointTensorMap{S}; alg::OFA = LQpos()) where {S<:ElementarySpace} =
    map(adjoint, reverse(leftorth!(adjoint(t); alg = alg')))

leftnull!(t::AdjointTensorMap{S}; alg::OFA = QR(), kwargs...) where {S<:ElementarySpace} =
    adjoint(rightnull!(adjoint(t); alg = alg', kwargs...))

rightnull!(t::AdjointTensorMap{S}; alg::OFA = LQ(), kwargs...) where {S<:ElementarySpace} =
    adjoint(leftnull!(adjoint(t); alg = alg', kwargs...))

function tsvd!(t::AdjointTensorMap{S};
                trunc::TruncationScheme = NoTruncation(),
                p::Real = 2,
                alg::Union{SVD, SDD} = SDD()) where {S<:ElementarySpace}
    u, s, vt, err = tsvd!(adjoint(t); trunc = trunc, p = p, alg = alg)
    return adjoint(vt), adjoint(s), adjoint(u), err
end

function leftorth!(t::TensorMap{<:ElementarySpace};
...
function leftnull!(t::TensorMap{<:ElementarySpace};
...
function rightorth!(t::TensorMap{<:ElementarySpace};
...
function rightnull!(t::TensorMap{<:ElementarySpace};
...
# function tsvd!(t::TensorMap{<:ElementarySpace};
#                 trunc::TruncationScheme = NoTruncation(),
#                 p::Real = 2,
...
function tsvd!(t::TensorMap{<:ElementarySpace};
    ...
    Udata = SectorDict{I, A}()
    # Σmdata = SectorDict{I, Ar}() # this will contain the singular values as matrix
    Σmdata = SectorDict{I, singularmatrixtype(Ar)}()
    Vdata = SectorDict{I, A}()
    dims = SectorDict{sectortype(t), Int}()
    if isempty(blocksectors(t))
        W = S(dims)
        truncerr = zero(real(scalartype(t)))
        return TensorMap(Udata, codomain(t)←W), DiagonalMap(Σmdata, W←W),
                    TensorMap(Vdata, W←domain(t)), truncerr
    end
    for (c, b) in blocks(t)
        U, Σ, V = _svd!(b, alg)
...
singularmatrixtype(TT::Type{<:AbstractMatrix}) = Core.Compiler.return_type(Diagonal, Tuple{TT})

function LinearAlgebra.ishermitian(t::TensorMap)
    domain(t) == codomain(t) || return false
    spacetype(t) <: ElementarySpace || return false # hermiticity only defined for euclidean
    for (c, b) in blocks(t)
...
function LinearAlgebra.isposdef!(t::TensorMap)
    ...
    spacetype(t) <: ElementarySpace || return false
    for (c, b) in blocks(t)
