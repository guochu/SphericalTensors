...
function Base.iterate(it::FusionTreeIterator{I, 1},
    ...
    state && return nothing
    tree = FusionTree{I, 1, 0}(it.uncoupled, it.coupled, it.isdual, ())
    return tree, true
end


#   General case:
function Base.iterate(it::FusionTreeIterator) 
    next = _iterate(it.uncoupled, it.coupled)
    next === nothing && return nothing
    lines, vertices, states = next
    f = FusionTree(it.uncoupled, it.coupled, it.isdual, lines)
    return f, (lines, vertices, states)
end
function Base.iterate(it::FusionTreeIterator, state)
