...
Base.@pure _sectors(::Type{T}) where {T<:SectorTuple} =
    (tuple_type_head(T),  _sectors(tuple_type_tail(T))...)

Base.IteratorSize(::Type{SectorValues{ProductSector{T}}}) where {T<:SectorTuple} =
    Base.IteratorSize(Iterators.product(map(values, _sectors(T))...))
...
function Base.iterate(::SectorValues{ProductSector{T}}, args...) where {T<:SectorTuple}
    next = iterate(Iterators.product(values.(_sectors(T))...), args...)
...
Base.one(::Type{ProductSector{T}}) where {I<:Sector, T<:Tuple{I, Vararg{Sector}}} =
    one(I) ⊠ one(ProductSector{tuple_type_tail(T)})
...
function ⊗(p1::P, p2::P) where {P<:ProductSector}
    if FusionStyle(P) isa UniqueFusion
        (P(first(Iterators.product(map(⊗, p1.sectors, p2.sectors)...))),)
    else
        return SectorSet{P}(Iterators.product(map(⊗, p1.sectors, p2.sectors)...))
    end
end
...
_isreal(T::Type{<:SectorTuple}) =
    isreal(tuple_type_head(T)) && _isreal(tuple_type_tail(T))
...
⊠(I::Type{<:Sector}, ::Type{ProductSector{T}}) where {T<:SectorTuple} =
    ProductSector{tuple_type_cons(I, T)}
