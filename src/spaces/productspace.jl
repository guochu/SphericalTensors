"""
    struct ProductSpace{S<:ElementarySpace, N} <: VectorSpace
...
spacetype(::Type{<:ProductSpace{S}}) where S = S
spacetype(V::ProductSpace) = spacetype(typeof(V)) # = spacetype(typeof(V))
sectortype(P::Type{<:ProductSpace}) = sectortype(spacetype(P))
...
"""
sectors(P::ProductSpace) = Iterators.product(map(sectors, P.spaces)...)
...
scalars.
"""
function insertunit(P::ProductSpace, i::Int = length(P)+1; dual::Bool = false, conj::Bool = false)
    u = oneunit(spacetype(P))
    if dual
        u = SphericalTensors.dual(u)
    end
    if conj
        u = SphericalTensors.conj(u)
    end
