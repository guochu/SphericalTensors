...
function ⊠(V::GradedSpace, P0::ProductSpace{<:ElementarySpace,0})
    I1 = sectortype(V)
    I2 = sectortype(P0)
    return Vect[I1 ⊠ I2](ifelse(isdual(V), dual(c), c) ⊠ one(I2) => dim(V, c) for c in sectors(V); dual = isdual(V))
end

function ⊠(P0::ProductSpace{<:ElementarySpace,0}, V::GradedSpace)
    I1 = sectortype(P0)
    I2 = sectortype(V)
    return Vect[I1 ⊠ I2](one(I1) ⊠ ifelse(isdual(V), dual(c), c) => dim(V, c) for c in sectors(V); dual = isdual(V))
end

function ⊠(P::ProductSpace{<:ElementarySpace,0}, P0::ProductSpace{<:ElementarySpace,0})
    I1 = sectortype(P)
    I2 = sectortype(P0)
    return one(Vect[I1 ⊠ I2])
end

function ⊠(P::ProductSpace{<:ElementarySpace}, P0::ProductSpace{<:ElementarySpace,0})
    I1 = sectortype(P)
    I2 = sectortype(P0)
    S = Vect[I1 ⊠ I2]
    N = length(P)
...
function ⊠(P0::ProductSpace{<:ElementarySpace,0}, P::ProductSpace{<:ElementarySpace})
    I1 = sectortype(P0)
    I2 = sectortype(P)
    S = Vect[I1 ⊠ I2]
    N = length(P)
