...
VU₁ = (U1Space(0=>1, 1=>2, -1=>2),
        ...
        U1Space(0=>1, 1=>2, -1=>3),
        U1Space(0=>1, 1=>3, -1=>3)')
VCU₁ = (Rep[CU₁]((0,0)=>1, (0,1)=>2, 1=>1),
        Rep[CU₁]((0,0)=>3, (0,1)=>0, 1=>1),
        Rep[CU₁]((0,0)=>1, (0,1)=>0, 1=>2)',
        Rep[CU₁]((0,0)=>2, (0,1)=>2, 1=>1),
        Rep[CU₁]((0,0)=>2, (0,1)=>1, 1=>2)')
VSU₂ = (SU2Space(0=>3, 1//2=>1),
        SU2Space(0=>2, 1=>1),
        SU2Space(1//2=>1, 1=>1)',
        SU2Space(0=>2, 1//2=>2),
        SU2Space(0=>1, 1//2=>1, 3//2=>1)')

for V in (Vℤ₂, Vℤ₃, VU₁, VCU₁, VSU₂)
    V1, V2, V3, V4, V5 = V
    @assert V3 * V4 * V2 ≿ V1' * V5' # necessary for leftorth tests
    @assert V3 * V4 ≾ V1' * V2' * V5' # necessary for rightorth tests
end

spacelist = (Vℤ₂, Vℤ₃, VU₁, VCU₁, VSU₂)
...
for V in spacelist
    ...
    @timedtestset "Full trace: test self-consistency" begin
        t = TensorMap(rand, ComplexF64, V1 ⊗ V2' ⊗ V2 ⊗ V1')
    ...
    @timedtestset "Partial trace: test self-consistency" begin
        t = TensorMap(rand, ComplexF64, V1 ⊗ V2' ⊗ V3 ⊗ V2 ⊗ V1' ⊗ V3')
    ...
    @timedtestset "Trace and contraction" begin
        t1 = TensorMap(rand, ComplexF64, V1 ⊗ V2 ⊗ V3)
        t2 = TensorMap(rand, ComplexF64, V2' ⊗ V4 ⊗ V1')
        t3 = t1 ⊗ t2
    ...
    @timedtestset "Tensor product: test via tensor contraction" begin
        for T in (Float32, ComplexF64)
            t1 = TensorMap(rand, T, V2 ⊗ V3 ⊗ V1)
            t2 = TensorMap(rand, T, V2 ⊗ V1 ⊗ V3)
            t = @constinferred (t1 ⊗ t2)
    ...
    @timedtestset "Factorization" begin
        ...
        for T in (Float32, ComplexF64)
            ...
            ts = (TensorMap(rand, T, W), TensorMap(rand, T, W)')
            for t in ts
                @testset "leftorth with $alg" for alg in (TK.QR(), TK.QRpos(), TK.QL(), TK.QLpos(), TK.Polar(), TK.SVD(), TK.SDD())
                ...
                @testset "rightorth with $alg" for alg in (TK.RQ(), TK.RQpos(), TK.LQ(), TK.LQpos(), TK.Polar(), TK.SVD(), TK.SDD())
            ...
            t = TensorMap(rand, T, V1 ⊗ V1' ⊗ V2 ⊗ V2')
...
@timedtestset "Deligne tensor product: test via conversion" begin
    @testset for Vlist1 in (VU₁, VSU₂), Vlist2 in (VU₁, Vℤ₂)
