...
@timedtestset "Fusion trees for $(type_repr(I))" for I in sectorlist
    Istr = type_repr(I)
    N = 5
    out = ntuple(n->randsector(I), N)
    isdual = ntuple(n->rand(Bool), N)
    _in = rand(collect(⊗(out...)))
    numtrees = count(n->true, fusiontrees(out, _in, isdual))
    while !(0 < numtrees < 30)
        out = ntuple(n->randsector(I), N)
        _in = rand(collect(⊗(out...)))
        numtrees = count(n->true, fusiontrees(out, _in, isdual))
    end
    it = @constinferred fusiontrees(out, _in, isdual)
    ...
    @testset "Fusion tree $Istr: elementy artin braid" begin
        ...
        for _in = ⊗(out...)
            for i = 1:N-1
                for f in fusiontrees(out, _in, isdual)
    ...
    @testset "Fusion tree $Istr: braiding and permuting" begin
        f = rand(collect(fusiontrees(out, _in, isdual)))
        p = tuple(randperm(N)...,)
        ip = invperm(p)
    ...
    @testset "Fusion tree $Istr: merging" begin
        ...
        @test dim(in1)*dim(in2) ≈ sum(abs2(coeff)*dim(c) for c in in1 ⊗ in2
                                                    for (f,coeff) in TK.merge(f1, f2, c))

        for c in in1 ⊗ in2
            R = Rsymbol(in1, in2, c)
            trees1 = TK.merge(f1, f2, c)
            ...
            for (t, coeff) in TK.merge(f2, f1, c)
                trees2[t] = get(trees2, t, zero(valtype(trees2))) + coeff*R
            end
    ...
    @testset "Double fusion tree $Istr: permutation" begin
        for n = 0:2N
            ...
            d = @constinferred permute(f1, f2, p1, p2)
            @test dim(incoming) ≈ sum(abs2(coef)*dim(f1.coupled) for ((f1,f2), coef) in d)
            d2 = Dict{typeof((f1,f2)), valtype(d)}()
            for ((f1′,f2′), coeff) in d
                d′ = permute(f1′,f2′, ip1, ip2)
