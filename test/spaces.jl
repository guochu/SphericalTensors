...
@timedtestset "ProductSpace{U1Space}" begin
    V1, V2, V3, V4 = U1Space(0=>3, 1=>2), U1Space(1=>2), U1Space(0=>3, 2=>1), U1Space(1=>2)
    ...
    @test @constinferred(insertunit(P, 3; conj = true)) == V1 ⊗ V2 ⊗ oneunit(V1)' ⊗ V3 ⊗ V4
...
@timedtestset "Deligne tensor product of spaces" begin
    ...
    @test sectortype(@constinferred((V1 ⊗ V2) ⊠ V3)) == @constinferred(Irrep[SU₂ × ℤ{3}])
    @test dim((V1 ⊗ V2) ⊠ V3) == dim(V1) * dim(V2) * dim(V3)
    @test sectortype((V1 ⊗ V2) ⊠ V3 ⊠ V4) == Irrep[SU₂ × ℤ{3} × U₁]
